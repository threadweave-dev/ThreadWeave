...
* timeout policy.
* logical name;
* version or immutable implementation reference;

A Task does **not** represent an execution.

A Task is identified by a stable logical name and an immutable version.

A Job MUST reference an exact Task identity, including the version or
another immutable implementation reference.

Example:

documents.extract_text@2
...
A **Result** is the immutable outcome produced by one completed Execution.
...
- runtime-specific diagnostic information.

A Job may therefore be associated with multiple Execution Results.

The Job-level outcome is determined from its Executions according to the
Job state machine and execution policy.
...
An **Executor** is an execution component that runs user code for a
specific runtime or execution format.

An Executor:

- receives an Execution assignment;
- resolves the referenced Task implementation;
- prepares the execution environment;
- invokes user code;
- reports lifecycle and progress information;
- returns an execution outcome.

An Executor never schedules Jobs and never selects its own workload.
...
A **Runtime** is a language-specific integration that exposes ThreadWeave
to application developers.

A Runtime typically provides:

- a public SDK;
- Task declaration APIs;
- argument and result serialization;
- Task registration and discovery;
- Job submission and inspection APIs;
- protocol client functionality;
- packaging conventions for executable applications.
...
A Runtime does not execute orchestration decisions.

A Runtime distribution MAY include tooling or an Executor implementation,
but Runtime and Executor remain distinct architectural roles.
...
A **Command** is a request for ThreadWeave to perform an action.
